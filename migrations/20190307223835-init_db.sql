...
CREATE TABLE IF NOT EXISTS exercises (
  id SERIAL PRIMARY KEY,
  title VARCHAR(255) NOT NULL,
  exercise_type INTEGER NOT NULL REFERENCES exercise_types(id) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS workout_exercises (
  id SERIAL PRIMARY KEY,
  workout INTEGER NOT NULL REFERENCES workouts(id) ON DELETE CASCADE,
  exercise INTEGER NOT NULL REFERENCES exercises(id) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS workout_exercise_sets (
  id SERIAL PRIMARY KEY,
  workout_exercise INTEGER NOT NULL REFERENCES workout_exercises(id) ON DELETE CASCADE,
  measurement_unit INTEGER NOT NULL REFERENCES measurement_units(id) ON DELETE CASCADE,
  reps INTEGER,
  resistance FLOAT,
  duration INTEGER
);
